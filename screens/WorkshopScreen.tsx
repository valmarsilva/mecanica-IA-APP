...
const WorkshopScreen: React.FC<WorkshopScreenProps> = ({ onNavigate, onComplete, currentCode = 'P0301' }) => {
  ...
  return (
    <div className="flex flex-col min-h-full bg-slate-950 pb-20 relative">
      ...
      <div className="relative bg-slate-900 border-y border-slate-800 min-h-[300px] flex items-center justify-center overflow-hidden">
        {showTip && (
          <div className="absolute top-4 left-4 right-4 bg-white rounded-2xl shadow-2xl z-40 animate-in fade-in slide-in-from-top-4 duration-300">
            <div className="bg-amber-500 px-4 py-1.5 flex items-center gap-2">
              ...
              <span className="text-[9px] font-black text-white uppercase tracking-widest">Master Mechanic AI</span>
            </div>
          ...
          <div className="absolute inset-0 z-50 bg-slate-950 flex flex-col animate-in slide-in-from-right duration-500">
            <div className="p-6 border-b border-slate-800 flex justify-between items-center bg-slate-900">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-white font-bold uppercase text-xs tracking-widest">Ficha Técnica</h3>
              </div>
              <button onClick={() => setShowLesson(false)} className="p-2 text-slate-400 hover:text-white bg-slate-800 rounded-full"><X size={20} /></button>
            </div>
            <div className="flex-1 overflow-y-auto p-6">
              {loadingLesson ? (
                <div className="h-full flex flex-col items-center justify-center space-y-4">
                  <Loader2 size={40} className="text-blue-500 animate-spin" />
                  <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest">Consultando Banco Valtec...</p>
                </div>
              ) : (
        ...
        {step === 2 && (
          <div className="absolute inset-0 bg-slate-950/95 backdrop-blur-md flex flex-col items-center justify-center p-8 text-center space-y-6 z-50">
            ...
            <button onClick={onComplete} className="w-full py-4 bg-emerald-600 text-white font-black rounded-2xl shadow-xl active:scale-95 transition-all text-xs uppercase tracking-widest">Confirmar Entrega</button>
          </div>
        )}
      </div>
        ...
        {selectedPart ? (
          <div className="space-y-4">
            <div className="flex items-start justify-between">
              ...
              <button onClick={handleOpenLesson} className="bg-blue-500/10 p-3 rounded-xl text-blue-500 hover:bg-blue-600 hover:text-white transition-all"><BookOpen size={20} /></button>
            </div>
            <div className="flex gap-3">
              <button className="flex-1 py-3 bg-slate-900 text-slate-400 font-bold rounded-xl text-[10px] border border-slate-800 uppercase tracking-widest">Testar Valor</button>
              <button onClick={handleRepair} className="flex-2 px-8 py-3 bg-blue-600 text-white font-bold rounded-xl text-[10px] shadow-xl shadow-blue-600/20 active:scale-95 transition-all uppercase tracking-widest">Executar Reparo</button>
            </div>
          </div>
        ) : (
          <div className="h-full flex flex-col items-center justify-center text-slate-700 space-y-3 py-6 opacity-30">
            ...
            <p className="text-[10px] font-black uppercase tracking-widest text-center">Toque em um componente<br/>no painel central</p>
          </div>
        )}
      </div>
    </div>
